...
public static class Utils
{
    public static readonly Dictionary<string, (string hash, string url)> KnownModels = new()
    {
        ...
        {"models/hyperswap/hyperswap_1a_256.onnx", ("c0e98a8a03a238f461ed3d2570e426b49f46745ee400854a60dceeb70c246add", "https://huggingface.co/facefusion/models-3.3.0/resolve/main/hyperswap_1a_256.onnx?download=true") },
        {"models/hyperswap/hyperswap_1b_256.onnx", ("5124031789c42f71b9558fb71954ef7aedb6da7ed9fac79293e23c61a792a73e", "https://huggingface.co/facefusion/models-3.3.0/resolve/main/hyperswap_1b_256.onnx?download=true") },
        {"models/hyperswap/hyperswap_1c_256.onnx", ("5528c2d76fe9986c99d829278987ef9f3a630cb606db7628d02b57b330f406a5", "https://huggingface.co/facefusion/models-3.3.0/resolve/main/hyperswap_1c_256.onnx?download=true") },
    ...
    public static void DownloadOrValidateModel(WorkflowGenerator workflowGenerator, string relativePath)
    {
        ...
        foreach (var (id, data) in Program.Backends.AllBackends)
        {
            if (data.AbstractBackend.Status is BackendStatus.DISABLED or BackendStatus.ERRORED or BackendStatus.LOADING || data.AbstractBackend is not ComfyUISelfStartBackend backend) continue;
    ...
    // Cleans up parameters that are left as default
    public static bool GetAndRemoveIfDefault(this T2IParamInput input, T2IRegisteredParam<bool> param)
    {
        if (input.TryGet(FaceToolsExtension.RemoveParamsIfDefault, out var removeParams) && !removeParams) return input.Get(param);
        var defaultVal = bool.Parse(param.Type.Default);
        if (!input.TryGet(param, out var value)) return defaultVal;
        if (value == defaultVal)
            if (input.TryRemove(param.Type))
                Logs.Verbose($"{FaceToolsExtension.ExtensionPrefix}Removed redundant param '{param.Type.ID}' as it was set to the default");
